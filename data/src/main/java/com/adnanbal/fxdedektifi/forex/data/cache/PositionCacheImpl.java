...
public class PositionCacheImpl implements PositionCache {
  ...
  private PositionEntity getEntityFromCache(final String positionId) {
    ...
    return Observable.create(emitter -> {
      if (positionEntity != null) {
        ...
        if (!isCached(positionEntity.getId())) {
          ...
          //Todo : check
          PositionEntity entityFromCache = getEntityFromCache(positionEntity.getId());
          if (entityFromCache.isOpen() != positionEntity.isOpen()) {
            delete(positionEntity.getId());
            put(positionEntity);
          }
        }
