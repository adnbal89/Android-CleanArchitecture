...
import com.adnanbal.fxdedektifi.sample.data.exception.SignalNotFoundException;
...
public class SignalCacheImpl implements SignalCache {
  ...
  private SignalEntity getEntityFromCache(final int positionId) {
  ...
  @Override
  public Observable<SignalEntity> get(int signalId) {
  ...
  public Observable<Boolean> put(SignalEntity signalEntity) {
    return Observable.create(emitter -> {
      ...
      if (signalEntity != null) {
        final File signalEntityFile = this.buildFile(signalEntity.getPositionId());
        if (!isCached(signalEntity.getPositionId())) {
          ...
          //Todo : check
          SignalEntity entityFromCache = getEntityFromCache(signalEntity.getPositionId());
          if (entityFromCache.isOpen() != signalEntity.isOpen()) {
            delete(signalEntity.getPositionId());
            put(signalEntity);
          }
        }
      }
  ...
  @Override
  public boolean isCached(int signalId) {
   ...
   * @return A valid file.
   */
  private File buildFile(int signalId) {
